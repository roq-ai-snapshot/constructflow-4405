...
function ProjectViewPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<ProjectInterface>(
    ...
    () =>
      getProjectById(id, {
        relations: ['organization', 'task', 'user_project'],
      }),
  );
              ...
              {hasAccess('user_project', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Stack spacing={2} mb={8}>
                  <Flex justifyContent="space-between">
                    ...
                    <NextLink passHref href={`/user-projects/create?project_id=${data?.id}`}>
                      <Button colorScheme="blue" mr="4" as="a">
                        Create
                      </Button>
                    </NextLink>
                    ...
                    <Table variant="simple">
                      ...
                      <Tbody>
                        {data?.user_project?.map((record) => (
                          <Tr
                            cursor="pointer"
                            onClick={() => router.push(`/user-projects/view/${record.id}`)}
                            key={record.id}
                          >
                            <Td>
                              {hasAccess('user_project', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                                <NextLink
                                  onClick={(e) => e.stopPropagation()}
                                  passHref
                                  href={`/user-projects/edit/${record.id}`}
                                >
                                  <Button mr={2} as="a" variant="outline" colorScheme="blue" leftIcon={<FiEdit2 />}>
                                    Edit
                                  </Button>
                              ...
                              {hasAccess('user_project', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                                <IconButton
                                  onClick={(e) => {
                                    e.stopPropagation();
                                    user_projectHandleDelete(record.id);
                                  }}
                                  colorScheme="red"
                                  variant="outline"
                                  aria-label="edit"
